...
// query callback representing the query of a chaincode
func (t *SimpleChaincode2) get(stub shim.ChaincodeStubInterface) pb.Response {
	params := []string{"query","a"}
	queryArgs := make([][]byte, len(params))
	for i , arg := range params{
		queryArgs[i] = []byte(arg)
	}
	response := stub.InvokeChaincode("mycc",queryArgs,"mychannel")
	if response.Status != shim.OK {
		return shim.Error("Failed to query chaincode. Got error")
	}
